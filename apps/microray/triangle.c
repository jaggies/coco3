...
static
int intersectTriangle(struct Shape* shape, Ray* ray, float *tmax) {
...
static
void normalTriangle(struct Shape* shape, Hit* hit, Vec3 *n) {
    copy3(&((Triangle*) shape)->normal[0], n);
}

static void boundsTriangle(struct Shape* shape, Vec3* min, Vec3* max) {
    // TODO
}

static
void uvTriangle(struct Shape* shape, struct Hit* hit, Vec2 * uv) {
    Triangle* triangle = (Triangle*) shape;
...
Shape* createTriangle(
    ...
    if (!_triangleOps.intersect) {
        _triangleOps.intersect = intersectTriangle;
        _triangleOps.normal = normalTriangle;
        _triangleOps.uv = uvTriangle;
        _triangleOps.bounds = boundsTriangle;
    }
